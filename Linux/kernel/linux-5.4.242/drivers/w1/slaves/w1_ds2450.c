...
#ifdef HAVE_W1_ACTIVITY_COUNTER
		if (data->spi_last_acnt != sl->master->activity_counter) {
			dev_dbg(&sl->dev, "spi_acnt %llu diff +%llu\n", sl->master->activity_counter, sl->master->activity_counter - data->spi_last_acnt);
#endif
...
#ifdef HAVE_W1_ACTIVITY_COUNTER
			data->spi_last_acnt = sl->master->activity_counter;
		} else {
			dev_dbg(&sl->dev, "spi_acnt %llu\n", sl->master->activity_counter);
		}
#endif
...
#ifdef HAVE_W1_ACTIVITY_COUNTER
		data->spi_last_acnt = sl->master->activity_counter - 1;
#endif
	}
